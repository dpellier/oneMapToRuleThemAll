	    ...
	    _createClass(GoogleMap, [{
	        ...
	        value: function render() {
	            ...
	            this.points.forEach(function (point) {
	                var marker = _this.addPoint(point, {}, false);
	        ...
	        value: function addPoint(point) {
	            ...
	            var reloadCluster = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];

	            var options = Object.assign({}, this.options.marker, customOptions);

	            if (this.map) {
	                var _ret = (function () {
	                    var marker = new Marker(_this2.map, point, options);

	                    // Bind the info window on marker click if the option is set
	                    if (_this2.options.activeInfoWindow) {
	                        google.maps.event.addListener(marker, 'click', function () {
	                            _this2.infoWindow.open(point.data, _this2.map, marker);
	                    ...
	                    if (reloadCluster && _this2.plugins.clusterer && _this2.options.activeCluster) {
