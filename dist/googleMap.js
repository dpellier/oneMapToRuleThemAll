	    ...
	    _createClass(GoogleMap, [{
	        ...
	        value: function load(callback, loadingMask) {
	            ...
	            var urlParams = Object.keys(this.options.loadParams).reduce(function (param, key) {
	                param += key + '=' + _this2.options.loadParams[key] + '&';
	                return param;
	            }, '?');
	        ...
	        value: function focusOnMarker(markerId) {
	            ...
	            var zoom = arguments.length <= 3 || arguments[3] === undefined ? 0 : arguments[3];
	        ...
	        value: function addMarkers(points) {
	            ...
	            var _loop = function (i) {
	                options = objectAssign({}, _this3.options.marker, points[i].options ? points[i].options : {});

	                if (typeof options.activeInfoWindow === 'undefined' || options.activeInfoWindow === null) {
	                    options.activeInfoWindow = _this3.options.activeInfoWindow;
	                }
	                ...
	                var marker = new Marker(_this3.map, points[i], options);

	                // Bind the info window on marker click if the option is set
	                if (options.activeInfoWindow) {
	                    google.maps.event.addListener(marker, 'click', function () {
	                        _this3.infoWindow.open(points[i].data, _this3.map, marker);
	                    });
	                } else {
	                    google.maps.event.addListener(marker, 'click', function () {
	                        _this3.map.panTo(marker.getPosition());
	                    });
	                }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isHTMLElement = _require.isHTMLElement;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var InfoBoxPlugin = (function () {
	    function InfoBoxPlugin(options) {
	        ...
	        var infoBox = new InfoBubble(objectAssign({}, options, { content: '' }));
	    ...
	    function Marker(map, point, options) {
	        ...
	        if (marker.draggable) {
	            google.maps.event.addListener(this, 'drag', function (event) {
	                if (options.label) {
	                    _this.label.update(event.latLng);
	                }
	            });
